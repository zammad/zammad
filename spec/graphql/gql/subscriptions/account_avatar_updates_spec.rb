...
RSpec.describe Gql::Subscriptions::AccountAvatarUpdates, type: :graphql do
  ...
  context 'with authenticated user', authenticated_as: :target do
    ...
    it 'receives avatar updates for target user' do
      ...
      expect(mock_channel.mock_broadcasted_messages.first[:result]['data']['accountAvatarUpdates']['avatars'].count).to eq(1)
    end
