...
RSpec.describe Gql::Subscriptions::AccountDevicesUpdates, type: :graphql do
  let(:subscription) do
    <<~QUERY
  ...
  context 'with authenticated user', authenticated_as: :target do
    ...
    it 'receives user device updates for target user' do
      ...
      expect(mock_channel.mock_broadcasted_messages.first[:result]['data']['accountDevicesUpdates']['devices'].count).to eq(1)
    end
