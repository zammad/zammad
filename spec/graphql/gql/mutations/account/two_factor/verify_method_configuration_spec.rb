...
RSpec.describe Gql::Mutations::Account::TwoFactor::VerifyMethodConfiguration, type: :graphql do
  ...
  let(:mutation) do
    <<~MUTATION
      mutation accountTwoFactorVerifyMethodConfiguration($methodName: EnumTwoFactorAuthenticationMethod!, $payload: JSON!, $configuration: JSON!) {
        accountTwoFactorVerifyMethodConfiguration(methodName: $methodName, payload: $payload, configuration: $configuration) {
          recoveryCodes
          errors {
            message
            field
