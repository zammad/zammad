...
RSpec.describe Gql::Mutations::Account::TwoFactor::RemoveMethodCredentials, :aggregate_failures, type: :graphql do
  ...
  let(:mutation) do
    <<~GQL
      mutation accountTwoFactorRemoveMethodCredentials($methodName: String!, $credentialId: String!) {
        accountTwoFactorRemoveMethodCredentials(methodName: $methodName, credentialId: $credentialId) {
          success
        }
      }
    GQL
