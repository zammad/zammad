...
RSpec.describe Gql::Mutations::Ticket::Update, type: :request do
  ...
  describe 'trigger results in update response' do
    # Sync triggers are dispatched inside the update service's
      ...
      post '/graphql',
           params: {
             ...
             variables: {
               ...
               input:    { title: 'trigger me' },
             },
           },
           as:     :json

      expect(json_response.dig('data', 'ticketUpdate', 'ticket', 'priority', 'name')).to eq(high_priority.name)
    end
  end
end
