  ...
  describe '.verify!' do
    ...
    context 'when channel not exists' do
      it 'raises NoChannelError' do
        ...
        expect { described_class.verify!(options:) }.to raise_error(Whatsapp::Webhook::NoChannelError)
      end
    end
    ...
    context 'when hub.mode is not subscribe' do
      it 'raises VerificationError' do
        ...
        expect { described_class.verify!(options:) }.to raise_error(described_class::VerificationError)
      end
    end
    ...
    context 'when all options are valid' do
      it 'returns hub.challenge' do
        expect(described_class.verify!(options:)).to eq(options[:'hub.challenge'])
      end
    end
  end
end
