...
RSpec.describe Controllers::KnowledgeBase::AnswersControllerPolicy do
  ...
  # One action per state machine event, plus the generic update, as generated by the
  #   `has_publishing` route concern.
  let(:publishing_actions) do
    [:has_publishing_update].concat(CanBePublished::StateMachine.aasm.events.map { |event| :"has_publishing_#{event.name}" })
  end
