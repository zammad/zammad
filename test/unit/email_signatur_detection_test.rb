...
class EmailSignaturDetectionTest < ActiveSupport::TestCase
  ...
  test 'test case 1 - sender a' do
    ...
    fixture_files.each do |filepath, value|
      value[:content] = File.new( Rails.root.join('test', 'fixtures', filepath), 'r').read
    ...
    fixture_files.each_value do |value|
  ...
  test 'test case 3 - just tests' do
    ...
    message = File.new(Rails.root.join('test', 'fixtures', 'email_signature_detection', 'example1.html'), 'r').read
  ...
  test 'test case III - sender a - full cycle' do
    ...
    # process email I
    file = File.open(Rails.root.join('test', 'fixtures', 'email_signature_detection', 'client_a_1.txt'), 'rb')
    ...
    # process email II
    file = File.open(Rails.root.join('test', 'fixtures', 'email_signature_detection', 'client_a_2.txt'), 'rb')
    raw_email = raw_email_header + file.read
    ...
    # process email III
    file = File.open(Rails.root.join('test', 'fixtures', 'email_signature_detection', 'client_a_3.txt'), 'rb')
    raw_email = raw_email_header + file.read
