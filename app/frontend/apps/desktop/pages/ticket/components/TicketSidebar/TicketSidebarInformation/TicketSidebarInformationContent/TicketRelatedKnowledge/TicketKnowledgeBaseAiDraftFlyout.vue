...
const { requestDraft, isGenerating } = useTicketAiAssistanceEnqueueKnowledgeBaseAnswer(
  props.ticketId,
)
...
const isSuggestedAnswersListVisible = computed(
  () => showAiSuggestedAnswers.value && activeSidebar.value === 'information',
)
...
<template>
  <CommonFlyout
    :footer-action-options="{
      ...
      actionButton: { variant: 'submit', disabled: isGenerating || loading || pending },
    }"
    :name="name"
    @action="requestDraft"
  >
