...
import TicketSidebarGitHub from '#desktop/pages/ticket/components/TicketSidebar/TicketSidebarExternalIssueTracker/TicketSidebarGitHub/TicketSidebarGitHub.vue'
...
describe('TicketSidebarGitHub', () => {
  it('displays on ticket screen correctly', async () => {
    ...
    expect(
      wrapper.queryByRole('status', { name: 'Open Issues' }),
  ...
  it('displays the GitHub issue tracker', async () => {
    ...
    expect(
      await wrapper.findByRole('status', { name: 'Open Issues' }),
    ).toHaveTextContent('2')
