...
const createEligableTicketArticleData = (type: string) => {
...
describe.each(['telegram personal-message', 'sms'])(
  '%s action reply',
  (name) => {
    const createEligableData = () => createEligableTicketArticleData(name)

    describe('seeing possible article actions', () => {
      it(`cannot reply to ${name}, if sender is Agent`, () => {
        setupView('agent')
