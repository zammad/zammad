...
module Gql::Types::User::TaskbarItemEntity
  class KnowledgeBaseAnswerCreateType < Gql::Types::BaseObject
    ...
    # The publication state the draft would be created in, so the tab can show the same status
    #   icon a stored answer has. Out of the draft state, which is why it is nil for a tab that
    #   has not been through a form updater round trip yet.
    field :visibility, Gql::Types::Enum::KnowledgeBase::VisibilityType
    ...
    #   was declared with. Anything else is dropped rather than raised on: a single unexpected
    #   value in a stored draft would otherwise take the whole entity down with it, and with it the
    #   tab's title.
    def visibility
      value = @object['visibility'].presence&.to_sym

      value if VISIBILITIES.include?(value)
    end
  end
end
